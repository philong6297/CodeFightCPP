...
go viral! You can't let this ruin your reputation, so you want to apply the box blur algorithm to
the photo to hide its content.
...
pixel values from the 3 � 3 square that has its center at x, including x itself. All the pixels on
...
To get the value of the middle pixel in the input 3 � 3 square: (1 + 1 + 1 + 1 + 7 + 1 + 1 + 1 + 1)
...
other three integers are obtained the same way, then the surrounding integers are cropped from the
final result.
*/
std::vector<std::vector<int>> boxBlur(std::vector<std::vector<int>> image) {
    vector<vector<int>> ret(size(image) - 2, vector<int>(size(image[0]) - 2, 0));
    for (int i = 1; i < size(image) - 1; i++)
        for (int j = 1; j < size(image[i]); j++) {
            for (int y = i - 1; y <= i + 1; y++)
                for (int x = j - 1; x <= j + 1; x++)
                    ret[i - 1][j - 1] += image[y][x];
            ret[i - 1][j - 1] /= 9;
        }
    return ret;
}
